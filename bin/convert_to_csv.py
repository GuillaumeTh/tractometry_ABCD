...
From scil_convert_json_to_xlsx.py
"""
...
def _write_dataframes(dataframes, df_names, output_path):
    with open(output_path, "w") as writer:
        tmp = zip(dataframes, df_names[0])
        for i, df_zip in enumerate(tmp):
            df, df_name = df_zip
            df = df.reset_index()
            df = df.rename(columns={"index": "sid"})
            df = df.melt(id_vars=["sid"],
                         var_name="Bundle",
                         value_name="Value").dropna()
            df.insert(2, "Metric", df_name)
            df.insert(3, "Type", df_names[1][i])
            df.loc[df["Metric"].str.contains("afd"), "Metric"] = "afd_fixel"
            if i == 0:
...
def _parse_scalar_meanstd(stats, subs, bundles):
    ...
    return dataframes, (df_names, df_type)
